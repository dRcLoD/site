...
- 🎴 **卡组综合工具**：导入 YDK 卡组文件，支持卡组展示、随机抽卡、双卡组对决模拟、小世界现象计算，结果导出 CSV
...
支持三种模式（均在前端本地解析 YDK 文件）：

- **单卡组模式**：展示主/副/额外卡组组成，支持多轮随机抽卡模拟，导出 CSV（含卡组构成与各轮抽卡结果）。
- **双卡组对决模式**：载入两份卡组，随机决定先攻方，模拟多轮"各抽 5 张 + 先攻额外 1 张"，导出 CSV。
- **小世界现象计算器**：基于主卡组怪兽，支持「路线检索」（手卡展示怪兽 → 跳板 → 目标怪兽）与「跳板查找」两种查询；按种族/属性/星级/攻/守恰好一项相同判定连接，结果带共同项徽章。牌组内无跳板时自动从 ygocdb 全卡数据库懒加载检索（仅保留怪兽精简字段，常驻内存约 2-3MB，会话内缓存）。
